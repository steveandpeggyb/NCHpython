...
# print('Length of xRaw: ' + str(len(xRaw)), 'Length of yRaw: ' + str(len(yRaw)))

PrepForOutput = ()

for x in range(len(xRaw)):
    PrepForOutput = np.append(PrepForOutput, str(xRaw[x]) + '\t' + str(yRaw[x]))

print() 

# for x in range(len(xRaw)):
#     print(PrepForOutput[x])

with open('C:\\Users\\csb003\\Desktop\\PythonProjects\\Pi() projects\\pidata.csv', 'w') as writeFile:
    writer = csv.writer(writeFile)
    for x in range(len(PrepForOutput)):
        writer.writerows(PrepForOutput[x])
writeFile.close()
